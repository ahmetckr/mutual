...
if(!message.member.hasPermission('BAN_MEMBERS')) return fonksiyonlar.hata('Bu komutu kullanabilmek için `Üyeleri Yasakla` yetkisine sahip olman gerekiyor.', message)
if(!message.guild.member(client.user.id).hasPermission('BAN_MEMBERS')) return fonksiyonlar.hata('Bu komutu kullanabilmek için `Üyeleri Yasakla` yetkisine sahip olmam gerekiyor.', message)
...
if(kullanıcı.id === message.author.id) return fonksiyonlar.hata('Kendini yasaklayamazsın.', message)
...
exports.help = {
 name: 'ban',
 description: 'Sunucunuzdaki üyeyi yasaklar.',
 aliases: ['yasakla'],
 bölüm: 'yetkili',
 timeout: 5
};
